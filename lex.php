...
class Lexer
{
    ...
    private function getType($word)
    {
        ...
        if (preg_match("/^CALL$/i", $word) && $this->index == 0) return tokenType::T_CALL;
        if (preg_match("/^RETURN$/i", $word) && $this->index == 0) return tokenType::T_RETURN;
        ...
        if (preg_match("/^JUMP$/i", $word) && $this->index == 0) return tokenType::T_JUMP;
        if (preg_match("/^JUMPIFEQ$/i", $word) && $this->index == 0) return tokenType::T_JUMPIFEQ;
        if (preg_match("/^JUMPIFNEQ$/i", $word) && $this->index == 0) return tokenType::T_JUMPIFNEQ;
        ...
        if (preg_match("/^[\p{L}_\-$&%\*!?][\d\p{L}_\-$&%\*!?]*$/i", $word)) return tokenType::T_LABELNAME;
